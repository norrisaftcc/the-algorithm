...
**Keep:** One negotiator-seat model runs The Algorithm with the human. Fan-out occurs if and only if the program is frozen. Nothing below the waist negotiates. The waist is retained *because* the cost matrix inverted: idle agent labor is approximately free; human gate attention is the scarce input the whole protocol optimizes. The waist is per program chain, not global: a classroom is many parallel sandboxed chains, each with its own gate (see K17 and the pending signer-scoping amendment).
...
**Keep:** Freeze cascades down: an orchestrator decomposes a frozen program into sub-programs that inherit the freeze, floor-preserved and hash-pinned. Reopen cascades down the same way. Nothing cascades up: a child never freezes anything and never renegotiates upstream. During execution, the human's powers are exactly {read everything, end the workflow} — observe and abort, never patch. Abort reopens the program tree.
...
**Recorded 2026-07-31 — outside-context problem, routed upstairs:** K4 names two execution-side powers: read everything, and end the workflow. Interactive ultracode grants a third. The human may skip one agent from the progress tree. Skip leaves no receipt and does not reopen the program. Skip is therefore a live intervention K4 forbids in principle and the harness permits in fact. This tree cannot verify the routing of this finding, because the board sits outside the session. The claim is unverified: it comes from the ultracode-failure-modes run, which refuted six of six checked claims and left the skip claim unchecked. Route this entry to the board at the next K4 review.
**Status:** Negotiating — strong candidate.
**Reopens if:** An execution failure mode genuinely requires live patching (treat any such claim as guilty until proven); or the board rules on the skip power recorded above.

## K5 — Git is the gate's physical form; identity is the hash

**Keep:** Freeze = a commit or tag signed by the authorized human's key. Programs are content-hashed; children pin the hash. The log is the amendment record. A CI job diffs HEAD against founding commits and reports mass-loss direction (drift audit). Naming discipline follows: proposals are named by content-hash plus working title — cheap, unnumbered, disposable, collision-free. Ordinals exist only in the frozen ledger, allocated by the gate, on the trunk, in freeze order. Governs KEEP entries, ADRs in governed repos, and programs.
...
**Recorded 2026-07-30:** This working tree arrived by copy and paste, not by fork. Its history begins at the paste. The witness has a cut below its earliest commit, and prior attempts are unreachable from here. K5 claims the hash is identity; a pasted tree holds an identity that no upstream can confirm. The claim survives inside this tree and fails across the cut. Prior attempts are assumed to exist elsewhere. Reaching them is not scheduled.
**Recorded 2026-07-31:** Platform attribution records delegated acts under the delegating human's name. Observed: two pull-request events, one hour apart, same repository, same actor string — one act was the agent's by API call, one was the owner's by hand, and the records arrived identical in form. An event log's actor field therefore cannot establish who acted, in either direction. Freeze evidence remains the human's typed verb in session; a platform record corroborates, never establishes. A "the human did X" read from an event log alone scores as unverified (K6). The signed-freeze clause above is the repair: a signature distinguishes principal from delegate; an actor field does not.
...
- **Q1 — Scope: ANSWERED 2026-07-28.** Agent fan-outs building course modules, with every module artifact mapping module learning objectives back to course learning objectives. Traceability is part of each program's Scope and ships as residue (K6, K17).
