...
**Recorded 2026-07-31 — outside-context problem, routed upstairs:** K4 names two execution-side powers: read everything, and end the workflow. Interactive ultracode grants a third. The human may skip one agent from the progress tree. Skip leaves no receipt and does not reopen the contract. Skip is therefore a live intervention K4 forbids in principle and the harness permits in fact. This tree cannot verify the routing of this finding, because the board sits outside the session. The claim is unverified: it comes from the ultracode-failure-modes run, which refuted six of six checked claims and left the skip claim unchecked. Route this entry to the board at the next K4 review.
**Status:** Negotiating — strong candidate.
